              ...
              <tbody className="divide-y divide-gray-200">
                {adminEvents?.map((event: AdminEvent, index: number) => (
                  ...
                  >
                    ...
                    <td className="py-4 px-6">
                      <div
                        className={`inline-flex items-center gap-2 px-3 py-1 rounded-full text-sm font-medium border w-fit ${getStatusColor(event.status)}`}
                      >
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
          <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
            <div className="text-sm text-gray-600">
              Hiển thị {Math.min((adminFilters.offset || 0) + 1, adminTotal)} -{" "}
              {Math.min(
                (adminFilters.offset || 0) + adminFilters.limit!,
                adminTotal,
              )}{" "}
              của {adminTotal} sự kiện
            </div>
            ...
            <div className="flex items-center gap-2">
                ...
                {Array.from(
                  {
                    length: Math.min(
                      5,
                      Math.ceil(adminTotal / adminFilters.limit!),
                    ),
                  },
                  (_, i) => {
                    const currentPage =
                    ...
                    return (
                      <button
                        key={pageNum}
                        onClick={() =>
                          updateAdminFilters({ offset: pageOffset })
                        }
              ...
              <button
                ...
                disabled={
                  (adminFilters.offset || 0) + adminFilters.limit! >= adminTotal
                }
                className="flex items-center gap-2 px-4 py-2 bg-white border border-gray-200 rounded-lg text-sm font-medium text-gray-700 hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
              >
                Sau
